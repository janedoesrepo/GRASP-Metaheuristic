...
class StationOrientedStrategy(OptimizationStrategy):
    ...
    def assign_tasks(self, candidate_list: List[Task], ordering_rule: TaskOrderingRule, cycle_time: int) -> List[List[Task]]:
        ...
        while len(candidate_list):
            ...
            candidate_tasks = [task for task in candidate_tasks if compute_station_time(current_station + [task]) <= cycle_time]                    
            ...
            ordered_candidate_value_list = ordering_rule.order_tasks(candidate_tasks, current_station)
